...
def test_sales_agent_executes_sales_path(monkeypatch) -> None:
    monkeypatch.setenv("ENABLE_ANTHROPIC_AGENTIC_RUNTIME", "true")

    async def fake_run_tool_loop(self, **kwargs):
        assert kwargs["thinking_budget_override"] == 4500
        assert kwargs["tool_names"] == ["execute_case_action"]
        return {
            "final_answer": "Close follow-up is the next best move.",
    ...
    monkeypatch.setattr("brain.agents.sales_agent.AnthropicAgentRuntime.run_tool_loop", fake_run_tool_loop)
    ...
    result = asyncio.run(
        run_sales_agent(
            ...
            handoff={
                ...
                "context": {"lead_score": 85, "deal_value": 20000},
            },
        )
    )
    ...
    assert result["usage"]["total_tokens"] == 95
