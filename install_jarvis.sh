...
LABEL="com.kan.jarvis"
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TEMPLATE_PLIST="${ROOT_DIR}/com.kan.jarvis.plist"
LAUNCH_AGENTS_DIR="${HOME}/Library/LaunchAgents"
TARGET_PLIST="${LAUNCH_AGENTS_DIR}/${LABEL}.plist"
RUN_LOG_DIR="${ROOT_DIR}/.run/full_auto/logs"
STDOUT_LOG="${RUN_LOG_DIR}/jarvis_listener.out.log"
STDERR_LOG="${RUN_LOG_DIR}/jarvis_listener.err.log"
GUI_DOMAIN="gui/$(id -u)"
...
if [[ -z "${PYTHON_BIN}" ]]; then
  echo "[fail] No se encontró python3 ni .venv/bin/python" >&2
  exit 1
fi
...
for key in ("OPENAI_API_KEY", "JARVIS_CLIENT_ID"):
...
usage() {
  cat <<EOF
Usage: ./install_jarvis.sh <install|uninstall|status>
EOF
}
